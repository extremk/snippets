...
# 兼容甬哥 sb.sh 管理脚本的 JSON 结构
#
# 严格协议隔离 — 防关联:
#   实例1: 主IPv4      (主端口)   严格IPv4-only
#   实例2: 保留IPv4    (端口+1)   严格IPv4-only
#   实例3: 主IPv6      (端口+2)   严格IPv6-only
#   实例4: 保留IPv6    (端口+3)   严格IPv6-only
# ====================================================
...
# ── Python 生成 ──────────────────────────────────────────────
echo "" && echo "⚙️  生成配置（兼容 sb.sh 管理脚本）..."
...
is_1_10       = (sb_major == 1 and sb_minor == 10)

print(f"   v{sb_mm} | 新DNS: {use_new_dns} | 新路由: {use_new_route} | 1.10系: {is_1_10}")
...
# ═══════════════════════════════════════════════════════════
# 实例1 主IPv4: 在原始结构上注入严格 IPv4 约束
# 保留甬哥脚本需要的 outbounds/route 结构和行号位置
# ═══════════════════════════════════════════════════════════

def inject_strict_ipv4_into_original(cfg, bind_v4=None):
    """
    在原始 sb.json 结构上注入严格 IPv4 约束
    不替换 outbounds/route 结构，只修改关键参数
    保持甬哥 sb.sh 管理脚本的 jq 查询兼容性
    """
    # 1. 修改所有 direct outbound 的 domain_strategy
    for ob in cfg.get('outbounds', []):
        if ob.get('type') == 'direct':
            if is_1_10:
                # 1.10.x 使用 domain_strategy
                ob['domain_strategy'] = 'ipv4_only'
            # 绑定 IPv4（如果指定）
            if bind_v4:
                ob['inet4_bind_address'] = bind_v4
                ob['bind_address_no_port'] = True
            ob['udp_fragment'] = True

    # 2. 注入 DNS 策略
    #    对于 1.12+ 需要新格式，但我们尽量保持原结构
    #    关键：在 dns 配置中注入 strategy: ipv4_only
    if 'dns' not in cfg:
        cfg['dns'] = {}

    # 如果原始配置有 dns.servers，保留它们但加 strategy
    if use_new_dns:
        # 1.12+ 新格式：完全替换 dns 为安全的 ipv4_only
        cfg['dns'] = {
            "servers": [
        ...
        # 1.10/1.11: 在现有 dns 上注入 strategy
        cfg['dns']['strategy'] = 'ipv4_only'

    # 3. 在路由规则中注入 IPv6 拒绝规则
    if 'route' not in cfg:
        cfg['route'] = {}
    ...
    if use_new_route:
        # 1.11+: 在 sniff 和 hijack-dns 之后、其他规则之前插入 reject
        # 找到合适的插入位置（在 sniff 和 dns 规则之后）
        insert_pos = 0
        for i, rule in enumerate(rules):
            if rule.get('action') in ('sniff', 'hijack-dns') or rule.get('protocol') == 'dns':
                insert_pos = i + 1

        # 检查是否已经有我们的拒绝规则（避免重复插入）
        has_v6_reject = any(
            r.get('ip_version') == 6 and r.get('action') == 'reject'
            for r in rules
        )
        if not has_v6_reject:
            rules.insert(insert_pos, {"ip_version": 6, "action": "reject", "method": "default"})
            rules.insert(insert_pos + 1, {"ip_cidr": ["::/0"], "action": "reject", "method": "default"})

        # 1.12+ 需要 default_domain_resolver
        if use_new_dns:
            cfg['route']['default_domain_resolver'] = {"server": "ipv4-dns-cf"}

    # 4. 移除 endpoints (warp) 防止 IPv6 泄漏
...
def build_clean_ipv4(cfg, bind_v4):
    ...
    direct_ob = {
        ...
        "bind_interface": "eth0", "inet4_bind_address": bind_v4,
    ...
    if not use_new_route:
        cfg['outbounds'].append({"type": "block", "tag": "block-out"})
...
# ── 实例1: 主IPv4 (修改 sb.json，保留原始结构) ───────────
print(f"\n📋 实例1: 主IPv4 (注入严格IPv4约束，保留原始结构兼容 sb.sh)")
cfg1 = copy.deepcopy(original_config)
inject_strict_ipv4_into_original(cfg1)
...
# 同步到 sb10.json / sb11.json（甬哥脚本会读取这些）
for sf in [f"{CONFIG_DIR}/sb10.json", f"{CONFIG_DIR}/sb11.json"]:
    if os.path.exists(sf):
        try:
            with open(sf, 'r') as f:
                scfg = json.load(f)
            inject_strict_ipv4_into_original(scfg)
            with open(sf, 'w') as f:
                json.dump(scfg, f, indent=4, ensure_ascii=False)
            print(f"      ✅ 同步修改 {os.path.basename(sf)}")
        except:
            pass

# ── 实例2: 保留IPv4 (端口+1, 完全重建) ──────────────────
...
sections = []
for iid, ifile, icfg, connect_ip, ilabel, etype, eip in generated:
    ll = gen_links(icfg, connect_ip, ilabel)
    if etype == "ipv6":
        info = f"入口IPv4: {main_public_ipv4} → 出口IPv6: {eip}"
    else:
        info = f"出口IPv4: {eip}"
...
systemctl is-active --quiet sing-box && echo "   ├─ ✅ sing-box (主IPv4) - 运行中" || {
...
if [ -f "$INST_INFO" ]; then
    while IFS='|' read -r ID FILE LABEL; do
        ...
        systemctl is-active --quiet "$SVC" && echo "   ├─ ✅ $SVC ($LABEL) - 运行中" || {
            echo "   ├─ ❌ $SVC ($LABEL) - 失败"
...
echo "║         IPv4-only | DNS仅A | reject IPv6                    ║"
}
...
echo "╠════════════════════════════════════════════════════════════════╣"
echo "║  恢复原始: cp /etc/s-box/sb.json.original /etc/s-box/sb.json ║"
echo "║            systemctl restart sing-box                        ║"
echo "║  管理脚本: sb (甬哥脚本正常使用)                              ║"
echo "╚════════════════════════════════════════════════════════════════╝"
